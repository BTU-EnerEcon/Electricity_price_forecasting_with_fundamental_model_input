...
%% Options:
path = 'C:\Users\dines\OneDrive - b-tu.de\Desktop\HIWI\Load prediction data and code\';% path of load input
name_load = 'Load_DayAheadForecastAndActual_DL-LU_2016to2024';% name of file with load; file with 3 columns: time, forecast, actual
sheet_load = 'Sheet1'; % name of sheet with load data
name_holiday = 'holiday_germany_2016to2024'; % list of holiday dates which have to be noted
...
try
    ...
    try
      ...
      error(['Failed to read holiday file: ', ME.message]);
    end
    clear opts
...
% Displaying the first 10 rows of the improved forecast
disp('First 10 rows of the improved forecast:');
disp(timetable2table(head(forecast_day_ahead_summary(:, {'ImprLoadforecast'}), 10)));

% Saving the entire improved forecast to a CSV file
try
    writetimetable(forecast_day_ahead_summary(:, {'ImprLoadforecast'}), 'C:\Users\dines\OneDrive - b-tu.de\Documents\ImprovedForecast.csv');
    disp('Improved forecast has been saved to ImprovedForecast.csv');
catch ME
    disp(['Error saving ImprovedForecast.csv: ', ME.message]);
end

catch ME
    warning(['Problem in estimation and forecast with used model, period size ', char(l_rw), 'Error: ', ME.message]);
end
...
function [Data_seas_adj, forecast_vec] = myfun_adj_seas_holiday_ex(Data, forecast_vec, holiday)
weekday_data = weekday(Data.Time);
for t = 1:length(holiday.Time)
    ind_feiertag = find((day(Data.Time) == day(holiday.Time(t))) & month(Data.Time) == month(holiday.Time(t)) & year(Data.Time) == year(holiday.Time(t)));
...
for t = 1:length(holiday.Time)
    ind_feiertag = find((day(forecast_vec.Time) == day(holiday.Time(t))) & month(forecast_vec.Time) == month(holiday.Time(t)) & year(forecast_vec.Time) == year(holiday.Time(t)));
    weekday_data_forecast(ind_feiertag) = 8;
end
